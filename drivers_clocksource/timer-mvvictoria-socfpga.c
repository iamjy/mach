/*
 *  arch/arm/mach-mv89xx/timer-mvvictoria-socfpga.c
 *
...
#define DRV_NAME	"mvvictoria-timer"
...
static const struct of_device_id victoria_timer_ids[] __initconst = {
...
cycle_t victoria_timer_read(void)
{
	u64 tdone, tval;
...
static struct clocksource victoria_clocksource = {
	.name = "victoria_clocksource",
	.rating = 200,
	.read = victoria_timer_read,
	.mask = CLOCKSOURCE_MASK(16),
	.flags = CLOCK_SOURCE_IS_CONTINUOUS,
};

static irqreturn_t victoria_clocksource_irq(int irq, void *dev_id)
{
...
static struct irqaction victoria_clocksource_irqaction = {
	.name = "victoria_clocksource_handler",
	.flags = (IRQF_DISABLED | IRQF_TIMER),
	.handler = victoria_clocksource_irq,
};

static void victoria_clocksource_init(void)
{
	struct clocksource *cs = &victoria_clocksource;
	...
	timer = of_find_matching_node(sys_timer.path, &victoria_timer_ids[id]);
	...
	setup_irq(hwirq, &victoria_clocksource_irqaction);
...
static void victoria_timer_set_mode(enum clock_event_mode mode,
...
static int victoria_timer_set_next_event(unsigned long cycle,
		struct clock_event_device *ce)
{
	unsigned int id = sys_timer.clkevt_id;
...
static struct clock_event_device victoria_clockevent = {
	.name = "victoria_clockevent_device",
	.features = CLOCK_EVT_FEAT_PERIODIC | CLOCK_EVT_FEAT_ONESHOT,
	.rating = 200,
	.set_mode = victoria_timer_set_mode,
	.set_next_event	= victoria_timer_set_next_event,
};

static irqreturn_t victoria_clockevent_irq(int irq, void *dev_id)
{
	struct clock_event_device *evt = &victoria_clockevent;
...
static struct irqaction victoria_clockevent_irqaction = {
	.name = "victoria_clockevent_handler",
	.flags = (IRQF_DISABLED | IRQF_TIMER),
	.handler = victoria_clockevent_irq,
};

static void victoria_clockevent_init(void)
{
	struct clock_event_device *ce = &victoria_clockevent;
	struct device_node *timer = NULL;
	struct irq_data *irq_dt = NULL;
	const char *path = NULL;
	unsigned int virq;
	...
	setup_irq(hwirq, &victoria_clockevent_irqaction);
...
#else /* !defined(CONFIG_GENERIC_CLOCKEVENTS) */

static struct timer_device_id {
	const char *name;
} victoria_timer_ids[TIMER_CNT] = {
	{ .name = "pwm-timer0" },
	{ .name = "pwm-timer1" },
	{ .name = "pwm-timer2" },
	{ .name = "pwm-timer3" },
...
static unsigned long victoria_timer_offset(void)
{
	u32 cntb = sys_timer.cntb;
...
static irqreturn_t victoria_timer_irq(int irq, void *dev_id)
{
#if TIMER_MEASURE
...
static struct irqaction victoria_timer_irqaction = {
	.name = "timer",
	.flags = (IRQF_DISABLED | IRQF_TIMER),
	.handler = victoria_timer_irq,
};

static void victoria_timer_hw_init(void)
{
	struct device_node *timer = NULL;
	struct irq_data *irq_dt = NULL;
	...
	err = of_property_read_string(of_aliases, victoria_timer_ids[id].name, &path);
	...
	setup_irq(hwirq, &victoria_timer_irqaction);
...
void __init victoria_timer_init(void)
{
	...
	if (WARN(err, "Failed to find mv,victoria-systimer property!"))
		return;
	...
	timer = of_find_matching_node(sys_timer.path,
								  &victoria_timer_ids[sys_timer.clkevt_id]);
	if (timer != NULL) {
	...
	timer = of_find_matching_node(sys_timer.path,
								  &victoria_timer_ids[sys_timer.clksrc_id]);
	if (timer != NULL) {
		if (of_property_read_u32(timer, "interrupts",
	...
	victoria_clockevent_init();
	victoria_clocksource_init();
#else 
	sys_timer.clk_id = PWM_ID_0;

	timer = of_find_matching_node(sys_timer.path,
								  &victoria_timer_ids[sys_timer.clk_id]);
	if (timer != NULL) {
		if (of_property_read_u32(timer, "interrupts",
									  &sys_timer.clkevt_hwirq)) {
	...
	victoria_timer_hw_init();
#endif /* CONFIG_GENERIC_CLOCKEVENTS */

	pr_info("[TIMER] IOMAP 0x%p\n", (void *)sys_timer.base); 
}

CLOCKSOURCE_OF_DECLARE(victoria_socfpga_timer, "mv,pwm-timer",
					   victoria_timer_init);
