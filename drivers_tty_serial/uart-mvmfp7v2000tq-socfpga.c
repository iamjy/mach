/*
 *  drivers/serial/uart-mvmfp7v2000tq-socfpga.c
 *
...
#if defined(CONFIG_SERIAL_MVT12SOCFPGA_CONSOLE) && defined(CONFIG_MAGIC_SYSRQ)
...
#define DRV_NAME		"mvmfp7v2000tq-uart"
#define MVFORMULA_MAX_UART	3
#define MV_DEVICENAME		"ttyS"

#ifdef CONFIG_OF
#if defined (CONFIG_MFP_7V2000T_Q_DEBUG_UART0)
static unsigned int uart_id = 0;
#elif defined(CONFIG_MFP_7V2000T_Q_DEBUG_UART1)
static unsigned int uart_id = 1;
#elif defined(CONFIG_MFP_7V2000T_Q_DEBUG_UART2)
static unsigned int uart_id = 2;
#elif defined(CONFIG_MFP_7V2000T_Q_DEBUG_UART3)
...
static void mv_uart_config_port(struct uart_port *port, int flags)
{
	...
	up->port.type = PORT_MVMFP7V2000TQ;
}
				...
				struct serial_struct *ser)
{
	...
	if ((ser->type != PORT_UNKNOWN) && (ser->type != PORT_MVMFP7V2000TQ)) {
		ret = -EINVAL;
		goto out;
	}
							  ...
							  struct mv_uart_port *up)
{
	...
	if (board_data->mapbase == UART0_BASE)
		up->name = "mvmfp7v2000tq-fpga-uart0";
	else if (board_data->mapbase == UART1_BASE)
		up->name = "mvmfp7v2000tq-fpga-uart1";
	else if (board_data->mapbase == UART2_BASE)
		up->name = "mvmfp7v2000tq-fpga-uart2";
	else if (board_data->mapbase == UART3_BASE)
		up->name = "mvmfp7v2000tq-fpga-uart3";
	else {
		ret = -EINVAL;
		goto out;
	}
...
#ifdef CONFIG_SERIAL_MVT12SOCFPGA_CONSOLE
...
static struct of_device_id of_platform_mv_serial_table[] = {
	{ .compatible = "serial-uart0", .data = (void *)PORT_MVMFP7V2000TQ, },
	{ .compatible = "serial-uart1", .data = (void *)PORT_MVMFP7V2000TQ, },
	{ .compatible = "serial-uart2", .data = (void *)PORT_MVMFP7V2000TQ, },
	{ .compatible = "serial-uart3", .data = (void *)PORT_MVMFP7V2000TQ, },
...
static int mv_uart_probe(struct platform_device *pdev)
{
	...
	mv_uart_ports[id].name = "mvmfp7v2000tq-fpga-uart";
	sprintf(&idx, "%d", id);
	...
	switch (port_type) {
	case PORT_MVMFP7V2000TQ:
	{
		dev_info(&pdev->dev, "Port type: PORT_MVMFP7V2000TQ\n");
