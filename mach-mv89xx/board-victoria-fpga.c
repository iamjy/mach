/*
 *  arch/arm/mach-mv89xx/board-victoria-fpga.c
 *
...
#define dbg(format, arg...) \
	printk(KERN_ALERT "[VICTORIA] DBG: " format, ## arg)
#define enter()         \
	printk(KERN_ALERT "[VICTORIA] ENTER: %s\n", __func__)
#define leave()         \
	printk(KERN_ALERT "[VICTORIA] LEAVE: %s\n", __func__)
#elif (MVVICTORIA_BOARD_DEBUG_LEVEL ==12)
#define dbg(format, arg...) \
	printk(KERN_ALERT "[VICTORIA] DBG: " format, ## arg)
#define enter()
#define leave()
...
static const char *victoria_dt_board_compat[] = {
	"mtekvision,socfpga-victoria",
	NULL
};

DT_MACHINE_START(VICTORIA, "Mtekvision VICTORIA FPGA PLATFORM V1.0")
	.map_io			= mv_map_io,
	...
	.dt_compat		= victoria_dt_board_compat,
MACHINE_END
